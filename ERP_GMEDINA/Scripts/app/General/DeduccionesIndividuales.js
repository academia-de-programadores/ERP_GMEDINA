    ...
    _ajax(null,
        ...
        (data) => {
            ...
            for (var i = 0; i < ListaDeduccionIndividual.length; i++) {
                ...
                var botonDetalles = ListaDeduccionIndividual[i].dei_Activo == true ? '<button type="button" class="btn btn-primary btn-xs" id="btnDetalleAFP" data-id = "' + ListaDeduccionIndividual[i].dei_IdDeduccionesIndividuales + '">Detalles</button>' : '';

                //variable boton editar
                var botonEditar = ListaDeduccionIndividual[i].dei_Activo == true ? '<button type="button" class="btn btn-default btn-xs" id="btnEditarAFP" data-id = "' + ListaDeduccionIndividual[i].dei_IdDeduccionesIndividuales + '">Editar</button>' : '';

                //variable donde está el boton activar
                var botonActivar = ListaDeduccionIndividual[i].dei_Activo == false ? esAdministrador == "1" ? '<button type="button" class="btn btn-primary btn-xs" id="btnActivarAFP" afpid="' + ListaDeduccionIndividual[i].dei_IdDeduccionesIndividuales + '" data-id = "' + ListaDeduccionIndividual[i].dei_IdDeduccionesIndividuales + '">Activar</button>' : '' : '';
...
$("#btnEditDeduiccionIndividual").click(function () {
    // SIEMPRE HACER LAS RESPECTIVAS VALIDACIONES DEL LADO DEL CLIENTE
...
//Detalles//
///////////////////////////////////////////////////////////////////////////////////////////////////
$(document).on("click", "#IdexTable tbody tr td #btnDetalleDeduccionesIndividuales", function () {
    var ID = $(this).data('id');
    $.ajax({
        ...
        .done(function (data) {
            ...
            if (data) {
                ...
                $("#Detalles #afp_Aportedei_MontoInicialMinimoLps").html(data[0].dei_MontoInicial);
...
//Inactivar//
$(document).on("click", "#btnActivarDeduccionesIndividuales", function () {
    $("#EditarDeduccionesIndividuales").modal('hide');
