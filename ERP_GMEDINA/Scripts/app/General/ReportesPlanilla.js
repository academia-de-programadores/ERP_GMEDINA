...
$('#ReporteReportesVarios').on('change', function () {
...
$("#frmReportVariosPreview").bind('submit',function (e) {

    modelState = true;

    // validar reporte seleccionado    
    if ($('#ReporteReportesVarios').val() != 0) {
        $("#validation_ReporteRequerida").css('display', 'none');
        $("#AsteriscoReporte").removeClass('text-danger');
    ...
    if (modelState == true)
    {
        ...
        var data = $("#frmReportVariosPreview").serializeArray();

        $.ajax({
            url: "/ReportesPlanilla/ReportesVariosParametros",
            method: "POST",
            data: data
        })
    }
